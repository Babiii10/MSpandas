      ...
      expr = {
        ...
        if(is.na(RvarsPeakDetection$directory_rawData)|| 
          ...
          withProgress(message = 'Processing...', value = 0, {
            ...
            if (dir.exists(directoryOutput_NewRefMap)) {
              all_msdial <- list.files(directoryOutput_NewRefMap,
                                       pattern    = "\\.msdial$",
                                       full.names = TRUE, ignore.case = TRUE)
              # Exclure les fichiers AlignResult-* (fichiers projet MS-DIAL, pas des runs)
              all_msdial <- all_msdial[!grepl("AlignResult-", basename(all_msdial), ignore.case = TRUE)]
              # Ne garder que les .msdial sans CSV correspondant (vrais orphelins)
              ...
              msdial_stems <- tolower(sub("\\.msdial$", "", basename(all_msdial), ignore.case = TRUE))
              pending_msdial <- all_msdial[!(msdial_stems %in% existing_csv_stems)]
              
              if (length(pending_msdial) > 0) {
                message(sprintf(
                  "--- Reprise : %d fichier(s) .msdial sans CSV détectés → déconvolution par chunks ---",
                  length(pending_msdial)))
                # Traiter par chunks pour éviter crash BiocParallel sur gros lot
                ...
                for (ch in seq_len(n_chunks)) {
                  ...
                  message(sprintf("--- Reprise chunk %d/%d : %d fichier(s) ---",
                                  ch, n_chunks, length(chunk_files)))
            ...
            align_files <- list.files(directoryOutput_NewRefMap, pattern = "^AlignResult-", full.names = TRUE)
            ...
            path_file_msdial_NewRefMap <- list.files(directoryOutput_NewRefMap,
                                                     pattern = "\\.msdial$",
                                                     full.names = TRUE, ignore.case = TRUE)
            path_file_msdial_NewRefMap <- path_file_msdial_NewRefMap[
              !grepl("AlignResult-", basename(path_file_msdial_NewRefMap), ignore.case = TRUE)]
            
            if (length(path_file_msdial_NewRefMap) >= 1) {
              # .msdial restants = mode non-batché ou orphelins → déconvolution classique
              RvarsPeakDetection$peaks_MSDIAL_mono_iso_NewRefMap <- deconv_peaks_MSDIAL(
                path_to_peakList = req(path_file_msdial_NewRefMap),
                output_directory = directoryOutput_NewRefMap,
                file_adduct = "data/Adduit.csv",
                mass_slice_width = req(input$mass_slice_width_NewRefMap),
                min_PeaksMassif = req(input$minPeaksMassif_NewRefMap)
              ...
              # Mode batché : les .msdial ont déjà été traités et supprimés par batch.
              # Reconstituer le tableau agrégé à partir des CSV produits par after_batch_fun.
              ...
              if (length(csv_files) > 0) {
                ...
                if (length(all_peaks) > 0) {
                  ...
                  colnames(peaks_combined)[1:10] <- c("mz","mzmin","mzmax","rt","rtmin","rtmax",
                                                      "into","maxo","sn","sample")
                  RvarsPeakDetection$peaks_MSDIAL_mono_iso_NewRefMap <- peaks_combined
                }
              }
            }
