...
.run_msdial_batch_mode <- function(mzml_files, input_dir, output_files,
    ...
    for (i in seq_along(batch_groups)) {
      ...
      if (!is.null(shinyProgressData)) {
        updateShinyProgressBar(
          shinyProgressData = shinyProgressData,
          pbValue           = 3L,
          headerMsg         = sprintf("Peak picking — batch %d/%d...", i, n_batches),
          footerMsg         = sprintf("%d fichiers traités sur %d",
                                     min(i * batch_size, length(mzml_files)),
                                     length(mzml_files))
        )
      }
    }
